{
  ...
  cfg = config.hamra.home.programs.scripts.flatpak-install;
  ...
  '';
in {
  options.hamra.home.programs.scripts.flatpak-install = mkOption {
    type = types.bool;
    default = false;
    description = "Enable flatpak-install (FZF browser for Flathub).";
  };

  config.home.packages = mkIf cfg [flatpak-install];
}
