{
  ...
  cfg = config.hamra.home.programs.scripts.flatpak-remove;
  ...
  '';
in {
  options.hamra.home.programs.scripts.flatpak-remove = mkOption {
    type = types.bool;
    default = false;
    description = "Enable flatpak-remove (FZF browser to uninstall Flatpaks).";
  };

  config.home.packages = mkIf cfg [flatpak-remove];
}
