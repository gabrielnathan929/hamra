{
  ...
  cfg = config.hamra.programs.core.scripts."setup-ssh";
  ...
  '';
in {
  options.hamra.programs.core.scripts."setup-ssh" = mkOption {
    type = types.bool;
    default = true;
    description = "Enable setup-ssh interactive TUI helper.";
  };

  config.environment.systemPackages = mkIf cfg [setup-ssh];
}
